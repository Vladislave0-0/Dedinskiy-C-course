...
int graph_print(struct list* lst)
{
    ...
    for(int i = 0; i < lst->capacity; i++)
    {
        ...
        else
        {
            fprintf(lst->graph_file, "[style = filled, fillcolor = skyblue, color = black, shape = Mrecord, label =" 
                                     "\"{<f1> node_%d} | {<f2> prev = %d} | {<f3> value = NAN }| {<f4> next = %d }\"];\n", 
                                     i, lst->nodes_arr[i].prev, lst->nodes_arr[i].next);
        }
    }
    ...
    for(int i = 0; i < lst->capacity; i++)
    {
        if((lst->nodes_arr[i].next == -1) && (i != lst->head_node))
        {
            if(i == lst->tail_node)
            {
                fprintf(lst->graph_file, "\tnode%d:f2 -> node%d:f2 [color = \"red\"];\n", i, lst->nodes_arr[i].prev);
            }
        ...
        if((lst->nodes_arr[i].prev != -1) || (i == lst->head_node))
        {
            ...
            if(i != lst->head_node)
            {
                fprintf(lst->graph_file, "\tnode%d:f2 -> node%d:f2 [color = \"red\", splines=ortho];\n", i, lst->nodes_arr[i].prev);
            }

            fprintf(lst->graph_file, "\tnode%d:f4 -> node%d:f4 [color = \"seagreen\", splines=ortho];\n", i, lst->nodes_arr[i].next);
